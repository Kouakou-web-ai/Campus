  ...
  const onSubmit = async (data: ContactForm) => {
    try {
      ...
      try {
        ...
        console.error("Erreur d'envoi email FormSubmit:", emailErr);
      }

      // 3. Envoyer par email réel via notre file d'attente Nodemailer
      try {
        const { sendRealEmail } = await import('../../services/emailSender');
        await sendRealEmail(
          'truixk@gmail.com',
          `Nouveau message de contact CAMPUS - ${data.sujet}`,
          `<div style="font-family: sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px;">
             <h2 style="color: #4f46e5;">Nouveau message de contact</h2>
             <p>Vous avez reçu un message depuis le formulaire de contact public de CAMPUS :</p>
             <div style="background-color: #f8fafc; padding: 15px; border-radius: 8px; margin: 16px 0; border: 1px solid #cbd5e1; font-size: 13px;">
               <p style="margin: 4px 0;"><strong>Nom :</strong> ${data.nom}</p>
               <p style="margin: 4px 0;"><strong>Email :</strong> ${data.email}</p>
               <p style="margin: 4px 0;"><strong>Établissement :</strong> ${data.etablissement}</p>
               <p style="margin: 4px 0;"><strong>Sujet :</strong> ${data.sujet}</p>
             </div>
             <div style="background-color: #ffffff; padding: 15px; border-radius: 8px; border: 1px solid #cbd5e1; margin-top: 15px; font-size: 13px; white-space: pre-wrap;">
               <strong>Message :</strong><br/>
               ${data.message}
             </div>
             <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 20px 0;"/>
             <p style="color: #64748b; font-size: 11px;">Service de contact CAMPUS</p>
           </div>`
        );
      } catch (emailSenderErr) {
        console.error("Erreur lors de l'envoi de mail de contact réel:", emailSenderErr);
      }
