...
import { BookOpen, Clock, AlertCircle, Bell, GraduationCap, FileText, Eye, Download, X, ChevronLeft, ChevronRight, Video } from 'lucide-react';
...
export default function PortailApprentissage() {
  ...
  const { courses, assignments, scheduleEvents, grades, announcements, loading, students, resources, liveMeetings } = useRealtimeDataStore();
          ...
          {relevantLiveMeetings.map((meeting) => (
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-rose-500/10 text-rose-550 flex items-center justify-center flex-shrink-0 border border-rose-500/20">
                  <Video size={20} className="animate-pulse" />
                </div>
                <div>
                  <h4 className="text-[10px] font-extrabold text-rose-700 dark:text-rose-450 uppercase tracking-widest flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-rose-600 animate-ping inline-block" />
                    Cours en direct interactif
                  </h4>
                  <p className="text-xs font-bold text-slate-800 dark:text-slate-100 mt-0.5">
                    {meeting.courseName} <span className="font-normal text-slate-400">est dispensé en ligne par Prof. {meeting.teacherName} ({meeting.className})</span>
                  </p>
                </div>
              </div>

              <button
                onClick={() => navigate(`/app/visioconference/${meeting.id}`)}
                className="btn btn-xs btn-error text-white font-bold rounded-xl shadow-md shadow-rose-500/10 transition-transform active:scale-95 flex items-center gap-1"
              >
                Rejoindre la classe
              </button>
            </div>
        ...
        <>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-4">
              <h2 className="text-base font-semibold text-slate-800">Session actuelle</h2>
              ...
              <div className="card-premium p-6">
                ...
                {nextEvent ? (
                  <div className="bg-indigo-50 rounded-2xl p-4 animate-fade-up">
                    ...
                    <p className="text-xs text-indigo-500 mt-1">{nextEvent.room} · {nextEvent.dayOfWeek !== undefined ? DAYS[nextEvent.dayOfWeek] : 'Jour'} {nextEvent.startHour ?? 8}h</p>
                    <div className="mt-3 text-xs font-semibold text-indigo-600">
                      {nextEvent.startHour ?? 8}h00 – {(nextEvent.startHour ?? 8) + nextEvent.durationHours}h00
                    </div>
                  </div>
                ) : (
